...
case class UserSession(id: Long, session_key:String, user_id:Long, ipaddress:String, useragent:String, expiration_date:Timestamp)
...
class SessionTableDef(tag: Tag) extends Table[UserSession](tag, "session") {
  ...
  override def * =
    (id, session_key, user_id, ipaddress, useragent, expiration_date) <>(UserSession.tupled, UserSession.unapply)
